...
export const getApiKeysByUserId = listApiKeysByUser;
export const getApiKeyByKey = findApiKeyByHash;
export const findApiKeyByIdAsync = findApiKeyById;
export const createApiKeyRecord = createApiKey;

// Single-argument versions for backward compatibility
export function revokeApiKeyById(id: string): boolean {
  const db = getDb();
  const result = db.prepare('UPDATE api_keys SET is_active = 0 WHERE id = ?').run(id);
  return result.changes > 0;
}

export function deleteApiKeyById(id: string): boolean {
  const db = getDb();
